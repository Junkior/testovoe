...
class TestUserApiPage:
    @pytest.mark.parametrize("user_id, expected_result, valid_schema", [(2, 200, user_schemas.user_single_schema),
                                                                        (23, 404,
                                                                         user_schemas.single_user_not_found_schema)])
    ...
    def test_create_user(self, data_for_create_user):
        api_response = UserPage(url=self.url).create_user(valid_schema=user_schemas.create_user_schema,
                                                          body=data_for_create_user)
...
class TestUnknownApiPage:
    @pytest.mark.parametrize(
        "valid_schema, resource_id, expected_result",
        [(unknown_schemas.unknown_single_schema, 2, 200),
...
class TestRegister:
    @pytest.mark.parametrize(
        "body, valid_schema, expected_result",
...
class TestLogin:
    @pytest.mark.parametrize(
        ...
        [({"email": "eve.holt@reqres.in","password": "cityslicka"},
          login_schemas.successful_login_schema, 200),
         ({"email": "peter@klaven"}, login_schemas.unsuccessful_login_schema, 400)])
    def test_login(self, body, valid_schema, expected_result):
        api_response = LoginPage(url=self.url).login_user(valid_schema=valid_schema, body=body)
        assert api_response.status_code == expected_result